...
class SSDCNet_classify(nn.Module):
    def __init__(self, class_num,label_indice,div_times=2,\
        ...
        # fr_end = VGG16_frontend(5,4,True,False,False)
        # init parameters
        ...
        if self.frontend_name == 'VGG16':
            ...
            # self.front_end = torch.jit.trace(VGG16_frontend(block_num=self.block_num,decode_num=4,
            # load_weights=self.load_weights,bn=self.IF_pre_bn,IF_freeze_bn=self.IF_freeze_bn), x)
            # self.front_end = torch.jit.trace(fr_end)
    ...
    def forward(self,x):
        x = self.front_end(x)
        # return { 'conv1':x[0] ,'conv2': x[1],\
        #          'conv3':x[2] ,'conv4': x[3], 'conv5': x[4] }
        return x
